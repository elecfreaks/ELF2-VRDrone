...
static struct cfvert cfvert_baro; //!< State information for vertical filter
static float vertPosition = 1.9f;
...
static void cfvert_update_baro(struct cfvert *cf, float baro);
#endif
// End Richile
...
static struct PIOS_SENSORS_3Axis_SensorsWithTemp *mpu_data = NULL;
...
#if defined(PIOS_INCLUDE_BARO)
void altitudeHoldSettingsUpdated(__attribute__((unused)) UAVObjEvent *ev)
{
	AltitudeHoldSettingsData altitudeHoldSettings;
	AltitudeHoldSettingsGet(&altitudeHoldSettings);
	vertPosition = altitudeHoldSettings.AltitudePI.Ilimit;
	float baro;
	BaroSensorAltitudeGet(&baro);
	cfvert_reset(&cfvert_baro, baro, vertPosition);
}
#endif
...
int32_t AttitudeInitialize(void)
{
	...
	AltitudeHoldSettingsConnectCallback(altitudeHoldSettingsUpdated);
#endif
...
static void AttitudeTask(__attribute__((unused)) void *parameters)
{
...
#if defined(PIOS_INCLUDE_BARO)
	altitudeHoldSettingsUpdated(NULL);
#endif
	
    if (cc3d) {
#if defined(PIOS_INCLUDE_MPU6000)
        gyro_test    = PIOS_MPU6000_Driver.test(0);
        mpu_data = pios_malloc(sizeof(PIOS_SENSORS_3Axis_SensorsWithTemp) + sizeof(Vector3i16) * 2);
#elif defined(PIOS_INCLUDE_MPU6500)
		gyro_test	 = PIOS_MPU6500_Driver.test(0);
		mpu_data = pios_malloc(sizeof(PIOS_SENSORS_3Axis_SensorsWithTemp) + sizeof(Vector3i16) * 2);
#endif
	...
	// Add by Richile
	float dT;
	int32_t timeval = PIOS_DELAY_GetRaw();
    ...
    while (1) {
		...
		AccelStateData accelsData;
		AccelStateGet(&accelsData);
		float z_accel = calc_ned_accel(q, &accelsData.x);

		static float last_z_accel = 0;
		if(last_z_accel == 0)
		...
		cfvert_predict_pos(&cfvert_baro,last_z_accel*0.25f + z_accel*0.75f, dT);
		...
		//if(lastBaroTime == 0)
		//	lastBaroTime = PIOS_DELAY_GetRaw();			
		if (xQueueReceive(baroQueue, &ev, 0) == true) {						
			//float baro_dT = PIOS_DELAY_DiffuS(lastBaroTime) / 1000000.0f;
			//PositionStateEastSet(&baro_dT);
			//PositionStateNorthSet(&cfvert_baro.baro_zero);
			//lastBaroTime = PIOS_DELAY_GetRaw();
			//PositionStateEastSet(&z_accel);
			BaroSensorAltitudeGet(&baro);
			cfvert_update_baro(&cfvert_baro, baro);
		}
		PositionStateDownSet(&cfvert_baro.position_z);
		VelocityStateDownSet(&cfvert_baro.velocity_z);
		// End Richile
#endif
...
static int32_t updateSensorsCC3D(AccelStateData *accelStateData, GyroStateData *gyrosData)
{
...
#elif defined(PIOS_INCLUDE_MPU6500)
	xQueueHandle queue = PIOS_MPU6500_Driver.get_queue(0);
#endif

    BaseType_t ret     = xQueueReceive(queue, (void *)mpu_data, sensor_period_ms);
    while (ret == pdTRUE) {
        gyros[0]  += mpu_data->sample[1].x;
        gyros[1]  += mpu_data->sample[1].y;
        gyros[2]  += mpu_data->sample[1].z;

        accels[0] += mpu_data->sample[0].x;
        accels[1] += mpu_data->sample[0].y;
        accels[2] += mpu_data->sample[0].z;

        temp += mpu_data->temperature;

        count++;
        // check if further samples are already in queue
        ret = xQueueReceive(queue, (void *)mpu_data, 0);
    }
    ...
    // gyrosData->temperature  = 35.0f + ((float)mpu_data.temperature + 512.0f) / 340.0f;
    // accelsData->temperature = 35.0f + ((float)mpu_data.temperature + 512.0f) / 340.0f;
	...
	//static uint32_t enableDisarmLastTime = 0;
	//uint8_t enableDisarm = false;
	//FlightStatusEnableDisarmGet(&enableDisarm);

	float z_accel = calc_ned_accel(q, &(accelStateData->x));
	//if((sqrtf(accelStateData->x*accelStateData->x+accelStateData->y*accelStateData->y+accelStateData->z*accelStateData->z)
	//	-9.81f) > 60.0f){
	if(z_accel > 240.0f){
		...
		if(armed == FLIGHTSTATUS_ARMED_ARMED){
			...
			uint8_t enableDisarm = true;
#else
			uint8_t enableDisarm = true;
#endif
			//enableDisarmLastTime = PIOS_DELAY_GetRaw();
			FlightStatusEnableDisarmSet(&enableDisarm);
			float throttle = -0.1f;
			StabilizationDesiredThrustSet(&throttle);
		}
	}
	//if(enableDisarm && (PIOS_DELAY_DiffuS(enableDisarmLastTime)/1000000.0f > 2.0f)){
	//	enableDisarm = false;
	//	FlightStatusEnableDisarmSet(&enableDisarm);
	//}
...
#elif defined(PIOS_INCLUDE_MPU6500)
		PIOS_MPU6500_Driver.get_scale(scales, 2, 0);
#endif
...
static void cfvert_update_baro(struct cfvert *cf, float baro)
{
	...
	cf->position_error_z = down - cf->position_z;
}
